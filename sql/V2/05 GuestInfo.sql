...
SELECT TOP 100 PERCENT
	...
	[RoomTypeOk] = CONVERT(BIT, ISNULL(CASE
	...
	[SharingWithId] = g2.[Id],
	[SharingWithOk] = CONVERT(BIT, ISNULL(CASE
			WHEN g.[Staff] = 1 AND r.[GuestId2] IS NOT NULL THEN 1
			...
			WHEN w.[Ticket_type] LIKE N'SINGLE ROOM: %' AND r.[GuestId2] IS NULL THEN 1
		END, 0)),
	[DietaryInfo] = g.[DietaryInfo],
	[DietaryInfoOk] = CONVERT(BIT, ISNULL(CASE
...
BEGIN
	...
	SELECT (
			SELECT *,
				[AllOk] = ISNULL([RoomTypeOk] & [RoomConfigOk] & [ReservationOk] & [SharingWithOk] & [DietaryInfoOk], 0)
			FROM [GuestInfo]
			ORDER BY [FullName]
			FOR JSON PATH
		)
